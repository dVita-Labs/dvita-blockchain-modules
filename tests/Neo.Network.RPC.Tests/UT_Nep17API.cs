...
namespace Neo.Network.RPC.Tests
{
    ...
    public class UT_Nep17API
    {
        ...
        public async Task TestGetTokenInfo()
        {
            ...
            scriptHash = NativeContract.DVITA.Hash;
            testScript = Concat(
                scriptHash.MakeScript("symbol"),
                scriptHash.MakeScript("decimals"),
                scriptHash.MakeScript("totalSupply"));
            UT_TransactionManager.MockInvokeScript(rpcClientMock, testScript,
                new ContractParameter { Type = ContractParameterType.String, Value = NativeContract.DVITA.Symbol },
                new ContractParameter { Type = ContractParameterType.Integer, Value = new BigInteger(NativeContract.DVITA.Decimals) },
                new ContractParameter { Type = ContractParameterType.Integer, Value = new BigInteger(1_00000000) });
            ...
            foreach (var test in tests)
            {
                ...
                else if (test.Request.Params[0].AsString() == NativeContract.DVITA.Hash.ToString() || test.Request.Params[0].AsString().Equals(NativeContract.DVITA.Name, System.StringComparison.OrdinalIgnoreCase))
                {
                    var result = await nep17API.GetTokenInfoAsync(NativeContract.DVITA.Name.ToLower());
                    Assert.AreEqual(NativeContract.DVITA.Symbol, result.Symbol);
                    Assert.AreEqual(0, (int)result.Decimals);
                    Assert.AreEqual(1_00000000, (int)result.TotalSupply);
                    Assert.AreEqual("NeoToken", result.Name);

                    result = await nep17API.GetTokenInfoAsync(NativeContract.DVITA.Hash);
                    Assert.AreEqual(NativeContract.DVITA.Symbol, result.Symbol);
                    Assert.AreEqual(0, (int)result.Decimals);
                    Assert.AreEqual(1_00000000, (int)result.TotalSupply);
                    Assert.AreEqual("NeoToken", result.Name);
