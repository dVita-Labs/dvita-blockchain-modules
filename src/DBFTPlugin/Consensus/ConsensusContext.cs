                ...
                Block = new Block
                {
                    Header = new Header
                    {
                        ...
                        NextConsensus = Contract.GetBFTAddress(
                            DvitaToken.ShouldRefreshCommittee(height + 1, neoSystem.Settings.CommitteeMembersCount) ?
                            NativeContract.DVITA.ComputeNextBlockValidators(Snapshot, neoSystem.Settings) :
                            NativeContract.DVITA.GetNextBlockValidators(Snapshot, neoSystem.Settings.ValidatorsCount))
                    }
                };
                var pv = Validators;
                Validators = NativeContract.DVITA.GetNextBlockValidators(Snapshot, neoSystem.Settings.ValidatorsCount);
