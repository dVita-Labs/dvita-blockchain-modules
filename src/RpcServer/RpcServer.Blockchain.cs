...
namespace Neo.Plugins
{
    partial class RpcServer
    {
        ...
        {
            using var snapshot = system.GetSnapshot();
            var validators = NativeContract.DVITA.GetNextBlockValidators(snapshot, system.Settings.ValidatorsCount);
            var candidates = NativeContract.DVITA.GetCandidates(snapshot);
        ...
        {
            return new JArray(NativeContract.DVITA.GetCommittee(system.StoreView).Select(p => (JObject)p.ToString()));
        }
