...
namespace Neo.Plugins
{
    public partial class RpcServer : IDisposable
    {
        ...
        private JObject ProcessRequest(HttpContext context, JObject request)
        {
            ...
            {
                return CreateErrorResponse(request["id"], -32602, $"Invalid params: FormatException: {e.ToString()}");
            }
            catch (IndexOutOfRangeException e)
            {
                return CreateErrorResponse(request["id"], -32602, $"Invalid params: IndexOutOfRangeException: {e.ToString()}");
            }
            catch (Exception ex)
            {
#if DEBUG
