...
namespace Neo.Network.RPC
{
    ...
    public class WalletAPI
    {
        ...
        {
            UInt160 scriptHash = NativeContract.DVITA.Hash;
        ...
        {
            BigInteger balance = await GetTokenBalanceAsync(NativeContract.DVITA.Hash.ToString(), account).ConfigureAwait(false);
            return (uint)balance;
        }
        ...
        public async Task<Transaction> ClaimGasAsync(KeyPair keyPair)
        {
            ...
            BigInteger balance = await nep17API.BalanceOfAsync(NativeContract.DVITA.Hash, toHash).ConfigureAwait(false);
            Transaction transaction = await nep17API.CreateTransferTxAsync(NativeContract.DVITA.Hash, keyPair, toHash, balance).ConfigureAwait(false);
